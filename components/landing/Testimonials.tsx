      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-20">
          <div className="inline-block px-4 py-1.5 rounded-full bg-white/5 border border-white/10 mb-6">
            <span className="text-[#0FB563] font-semibold text-sm tracking-wider uppercase">Testimonials</span>
          </div>
          <h2 className="text-4xl md:text-5xl font-bold text-white mb-6">
            Loved by teams <span className="text-[#0FB563]">worldwide</span>
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {testimonials.map((t, idx) => (
            <div key={idx} className="p-8 rounded-3xl bg-dark-2 border border-white/5 relative group hover:-translate-y-2 transition-transform duration-300 hover:shadow-[0_10px_40px_rgba(0,0,0,0.5)]">
              <div className="text-[#0FB563] text-6xl font-serif absolute top-4 right-6 opacity-20 group-hover:opacity-40 transition-opacity">"</div>
              <p className="text-gray-300 mb-8 relative z-10 text-lg leading-relaxed italic">
                "{t.quote}"
              </p>
              <div className="flex items-center gap-4">
                <div className="relative">
                  <div className="absolute inset-0 rounded-full bg-[#0FB563] blur-sm opacity-50 group-hover:opacity-100 transition-opacity" />
                ...
                <div>
                  ...
                  <p className="text-sm text-[#0FB563] font-medium">{t.role}</p>
                </div>
              </div>
            </div>
          ))}
